...
> **C++在编译阶段会对函数进行名字修饰, 通常以某种规则对 函数 + 形参类型 进行转换作为函数的新名字**
...
~~~C++
void func(int a = 11, int b = 12, int c = 13);
void func(int a, int b);
func(1, 1);//调用失败，两个func函数都可以精确匹配
~~~
...
`inline`函数与普通函数的区别在于，当编译器处理调用内联函数的语句时，不会将该语句编译成函数调用的指令，而是直接将整个函数体的代码插人调用语句处，就像整个函数体在调用处被重写了一遍一样。这一点有点类似于宏定义。中心思想是以空间换时间
...
- inline对于编译器而言**只是一个建议**，编译器会自动优化，**如果定义为inline的函数体内有循环/递归等**等，编译器优化时会忽略掉内联。
...
> 1. **编译处理方式不同**
>     #define，宏定义，是一条**预编译指令**, 编译器在预处理阶段会将所有使用到宏的地方简单地进行替换；
>     const常量是编译运行阶段使用；
>     因此，#define存在边际效应等问题，非常容易引起意想不到的错误，而且因为在预处理时就被替换，给调试则造成了很大困难；
> 2. **作用域**
...
>     const常量使用时，只是给出了对应的内存地址，而#define出的是立即数，所以，**const定义的常量在程序运行过程中只有一份拷贝**，而 **#define定义的常量在内存中有若干个拷贝**。#define宏空间利用率较低，在较长的字符串常量情况下更加明显。
> 5. 总结
>     定义常量时：
...
> 
> static 修饰的变量，不写入符号表，仅当前文件可见可用，且会改变生命周期

###### const

1、const修饰的量不是常量，仅仅是个只读量。在**编译的时候全部替换const变量被赋予的值**（这点和C语言的宏相似），在运行的时候该const变量可通过内存进行修改

> 1.1）通过内存(指针)可以修改位于栈区的const变量，语法合乎规定，编译运行不会报错，但是在编译的时候所有用到该常量的地方全部被替换成了定义时所赋予的值，然后再运行的时候无法使用通过指针修改后的值**(a 和  *pa 是不同的值)**。
>
> 1.2）通过内存(指针)修改**位于静态存储区的的const变量**，语法上没有报错，编译不会出错，**一旦运行就会报告异常(没有写入权限)**。
>
> **注：通过指针修改在全局区上的const变量，编译可通过，运行就会报异常。**

2、const volatile修饰的变量，可以在编译时不用全部替换被const volatile变量被赋予的值，因此可以在运行时使用通过内存修改后的值

> 2.1）通过内存(指针)可以修改位于栈区的const volatile变量，语法合乎规定，编译运行不会报错，在编译的时候所有用到该常量的地方不会替换成了定义时所赋予的值，在运行的时候可以使用通过指针修改后的值**(取消了编译器优化)**。
>
> 2.2）通过内存(指针)修改位于静态存储区的的const volatile变量，语法上没有报错，编译不会出错，**一旦运行就会报告异常(没有写入权限)**。
...
1. auto与指针和引用结合起来使用, 用auto声明指针类型时，用**auto和auto*没有任何区别**，但用**auto声明引用类型时则必须加&**
...
2. 在同一行定义多个变量, 当在同一行声明多个变量时，这些变量**必须是相同的类型**，否则编译器将会报错，因为编译器实际只对第一个类型进行推导，然后用推导出来的类型定义其他变量.
    ...
    void TestAuto()
    {
        ...
        //err：对于此实体auto类型是double， 但之前默认为int
    }
    ```
...
2. 类声明放在.h文件中，成员函数定义放在.cpp文件中。
    **注意：成员函数名前需要加 类名::，即域限定符，表示属于哪个作用域**![image-20221003112255057](%E5%9B%BE%E7%89%87/README/image-20221003112255057.png)
...
6. **无参的构造函数**和**全缺省的构造函数**都称为默认构造函数，并且**默认构造函数只能有一个**。注意：无参构造函数、全缺省构造函数、我们没写编译器默认生成的构造函数，都可以认为是默认构造函数
...
> [关于在cpp类构造函数中使用this指针 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/386532078#:~:text=this指针是可以用的，在构造函数初始化列表之前就已经有了this指针 只是构造还没完成，但是this指针已经可以用 只是在构造函数中用this的时候要谨慎,在构造函数的 {body}中 也就是在花括号之后的构造函数体内，使用this指针是安全的。)
...
> 只有单个形参，该形参**必须是**对**本类类型对象的引用**(一般常用const修饰)，在用已存在的同类型对象**创建新对象时**由编译器自动调用。

**其特征如下：**

1. **拷贝构造函数是构造函数的一个重载形式。**
2. 拷贝构造函数的参数只有一个且必须**使用引用传参**，使用传值方式会引发无穷递归调用。**//重点**
3. 若未显示定义，系统生成默认的拷贝构造函数。 默认的拷贝构造函数对象按内存存储**按字节序完成拷贝**，这种拷贝我们叫做**浅拷贝**，或者值拷贝。//如果申请了堆空间,则对应的指针会指向同一块空间
...
- 注意:
    ...
    6. **C++规定`=，[ ]，()，->`这四个运算符只能被重载为类的非静态成员函数，其他的可以被友元重载。▲▲**
    7. **赋值运算符在类中不显式实现时，编译器会生成一份默认的，此时用户在类外再将赋值运算符重载为全局的，就和编译器生成的默认赋值运算符冲突了，故赋值运算符只能重载成成员函数 ▲▲▲**

**特殊的运算符**

1. 不能重载的
    `.* 、:: 、sizeof 、?: 、.`  C++规定以上5个运算符不能重载
2. 只能重载成成员函数的
    `=，[ ]，()，->`  C++规定以上4个运算符只能重载为类的非静态成员函数，不能实现友元重载
...
2. **赋值运算符只能重载成类的成员函数不能重载成全局友元函数**
    ...
    Date& operator=(Date& left, const Date& right)//err
    {
     if (&left != &right)
     {
...
> 这两个运算符一般不需要重载，使用编译器生成的默认取地址的重载即可，只有特殊情况，才需要重载，比如想让别人获取到额外指定的内容！
...
> 匿名对象生命周期只有一行, 若**被const引用类型引用**, 则其生命周期会延长至引用的生命周期。
>
...
> 在创建对象时，编译器通过调用构造函数，给对象中各个成员变量赋一个合适的初始值。
>
...
> 初始化列表：以一个冒号开始，接着是一个以逗号分隔的数据成员列表，每个"成员变量"后面跟一个放在括 号中的初始值或表达式。**在申请内存时初始化，发生在构造函数调用**。
>
...
>
> **即构造函数 可以看作 具有类型转换的作用。explicit会禁止该功能**
...
void Test()
{
 ...
 d1 = 2023;//实际编译器背后会用2023构造出一个无名对象，最后用无名对象给d1对象进行赋值
 //将1屏蔽掉，2放开时则编译失败，因为explicit修饰构造函数时，禁止了单参构造函数类型转换的作用
 //即必须现行的构造，单变量不能再通过单参构造函数默认转换为对象
}
~~~
...
>
> **但是有特例：带有类内初始化表达式的静态数据成员必须具有不可变的常量整型类型，或必须被指定为"内联”**
>
> **即 `const static int a = 1;` 是合法语句 ： 常量整型类型 + 初始化 + static**

**特性:**

- 静态成员为所有类对象所共享，不属于某个具体的对象，存放在静态区
- **静态成员变量必须在类外定义**，定义时不添加static关键字，类中只是声明(有特例)
...
**特性：** 
 ...
 1. 内部类可以定义在外部类的public、protected、private都是可以的, **受访问限制符和外部类类域限制**。 
...
~~~C++
/*
operator new：该函数实际通过malloc来申请空间，当 malloc申请空间成功时直接返回；申请空间
...
delete[]在释放空间时，并不知道有多少个对象，所以d**elete[]会先找到int，拿到对象的个数**，再根据这个个数执行n次析构函数，保证对象全被析构，才释放空间
...
> - 但对于自定义类型，是需要析构函数的，对于new[]，调用op new时， **op new知道[]中的参数**，因此调用参数次的构造函数，但对于析构函数，delete[]中并不存在参数，不知道调用几次析构函数。所以，new[]在申请空间时，会在实际空间的头部**额外申请四个字节空间用来保存变量个数**，**delete[]会找到这个空间读取参数**。
...
- 函数模板是一个蓝图，它**本身并不是函数**，是编译器用来产生特定具体类型函数的模具。所以其实**模板就是将本来应该我们做的重复的事情交给了编译器**。

- 在**编译器编译阶段**，对于模板函数的使用，编译器需要根据传入的实参类型来推演生成对应类型的函数以供调用。(运行前多态)
...
模板参数列表, 和其函数模板, 类模板是一一对应的

1. 类模板/函数模板 只是一种声明, 并不是定义, 由编译器在编译阶段推导出来的**模板函数/模板类才是定义**

2. **类模板实例化只会实例化需要的部分**，若存在未用到函数，则不会实例化改函数▲
...
~~~C++
template<int T1, int T2, ..., int Tn>//a
class name
{
	// 类内成员定义--存在模板类型成员
};
template<T T1, T T2, ..., T Tn>//b
class name
{
    // 类内成员定义--存在模板类型成员
};
...
[(57条消息) C++ STL容器中不能存储引用类型的原因_KernelCc的博客-CSDN博客](https://blog.csdn.net/qq_38988226/article/details/110733006#:~:text=C%2B%2B STL容器中不能存储引用类型的原因 1 从设计目的上来讲，设计引用的目的是为了简化指针操作，避免指针造成的一系列问题，如果对引用再取地址的话，有违背设计初衷之意。 2,从实现角度来讲，引用常常为 指针常量 ，对指针常量取地址（指向指针常量的二重指针）本身便没有意义。 C%2FC%2B%2B 中定义二重指针往往是要对一重指针进行修改，而指针常量本身就不能修改，且倘若要访问引用所指变量的地址或者访问引用所指变量本身，直接可以通过引用完成，何必大费周折。)

[(57条消息) vector中不能存放引用类型!!!!!!!!_vector存放引用_Virtual_Func的博客-CSDN博客](https://blog.csdn.net/virtual_func/article/details/49724135)
...
> 基类定义了static静态成员，则**整个继承体系里面只有一个这样的成员**。**无论派生出多少个子类，都只有一个static成员实例** 。
>
...
3. 继承和组合
    ...
    - 继承允许你根据基类的实现来定义派生类的实现。这种通过生成派生类的复用通常被称 为**白箱复用(white-box reuse)**。术语“白箱”是相对可视性而言：在继承方式中，基类的内部细节对子类可见 。继承一定程度破坏了基类的封装，基类的改变，对派生类有很大的影响。派生类和基类间的依赖关系很强，耦合度高。 
    - 对象组合是类继承之外的另一种复用选择。新的更复杂的功能可以通过组装或组合对象 来获得。对象组合要求被组合的对象具有良好定义的接口。这种复用风格被称为**黑箱复用(black-box reuse)**，因为对象的内部细节是不可见的。对象只以“黑箱”的形式出现。 组合类之间没有很强的依赖关系，耦合度低。优先使用对象组合有助于你保持每个类被封装。 
    - 实际尽量多去用组合。组合的耦合度低，代码维护性好。不过继承也有用武之地的，有 些关系就适合继承那就用继承，另外要实现多态，也必须要继承。类之间的关系可以用组合，就用组合。
...
>     > 非虚继承，继承的是父类的，完整继承下来
>     >
>     > 虚继承+非虚继承，则会改变非虚继承的虚基表的内容，但个数不变
>     >
>     > 全虚继承，会生成自己的虚基表，指向父类，父类的不变，个数+1
>     >
>     > 抽象
...
> 多态是**同一个继承体系，不同继承关系的类对象，去调用同一函数，产生了不同的行为**。比如Student继承了 Person。Person对象买票全价，Student对象买票半价。 
>
...
3. **如果是private继承，则子类无法赋给父类的指针或引用(切片)，但可以强制类型转换，两个不同，但强转后也能构成多态**
...
2. **析构函数的重写(基类与派生类析构函数的名字不同)**
    ...
    如果基类的析构函数为虚函数，此时派生类析构函数只要定义，无论是否加virtual关键字， 都与基类的析构函数构成重写，虽然基类与派生类析构函数名字不同，看起来违背了重写的规则，其实不然，这里**可以理解为编译器对析构函数的名称做了特殊处理，编译后析构函数的名称统一处理成destructor。**

    **析构函数的重写，保证了基类指针指向子类对象时，调用析构函数，能够完成对子类资源的析构，保护了子类能够安全释放资源**。
...
[C++（纯）虚函数重写时访问权限更改问题](https://blog.csdn.net/t_t233333333/article/details/103411507)
...
> ~~~
>
> [(57条消息) 构造函数中为什么不能调用虚函数？_构造函数不能调用虚函数_线上幽灵的博客-CSDN博客](https://blog.csdn.net/chen134225/article/details/81564972)
>
> [(57条消息) 构造函数中为什么不能调用虚函数？_构造函数不能调用虚函数_线上幽灵的博客-CSDN博客](https://blog.csdn.net/chen134225/article/details/81564972)
...
> **在虚函数的后面写上 =0 ，则这个函数为纯虚函数。**包含纯虚函数的类叫做抽象类（**也叫接口类**），**抽象类不能实例化出对象**。**派生类继承后也不能实例化出对象，只有重写全部纯虚函数，派生类才能实例化出对象，否则派生类仍是抽象类，不能实例化对象**。纯虚函数规范了派生类必须重写，另外纯虚函数更体现出了接口继承。
...
通过观察测试我们发现b对象是**8bytes**，**除了_b成员，还多一个__vfptr放在对象的前面(注意有些 平台可能会放到对象的最后面，这个跟平台有关)，对象中的这个指针我们叫做虚函数表指针(v代 表virtual，f代表function)。**
...
6. 这里还有一个童鞋们很容易混淆的问题：虚函数存在哪的？虚表存在哪的？ 
    ...
    ![image-20230612175627614](%E5%9B%BE%E7%89%87/README/image-20230612175627614.png)
